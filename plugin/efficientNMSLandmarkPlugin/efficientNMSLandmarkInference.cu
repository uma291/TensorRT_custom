...
#include "efficientNMSLandmarkInference.cuh"
...
template <typename T>
__device__ float IOU(EfficientNMSLandmarkParameters param, BoxCorner<T> box1, BoxCorner<T> box2)
{
...
__device__ BoxCorner<T> DecodeBoxesLandmark(EfficientNMSLandmarkParameters param, int boxIdx, int anchorIdx,
...
template <typename T, typename Tb>
__device__ void MapNMSLandmarkData(EfficientNMSLandmarkParameters param, int idx, int imageIdx,
    const Tb* __restrict__ boxesInput, const T* __restrict__ landmarksInput, const Tb* __restrict__ anchorsInput,
    const int* __restrict__ topClassData, const int* __restrict__ topAnchorsData, const int* __restrict__ topNumData,
    const T* __restrict__ sortedScoresData, const int* __restrict__ sortedIndexData, T& scoreMap, int& classMap,
    BoxCorner<T>& boxMap, Landmark<T>& landmarkMap, int& boxIdxMap)
{
...
template <typename T>
__device__ void WriteNMSLandmarkResult(EfficientNMSLandmarkParameters param, int* __restrict__ numDetectionsOutput,
    T* __restrict__ nmsScoresOutput, int* __restrict__ nmsClassesOutput, BoxCorner<T>* __restrict__ nmsBoxesOutput,
    T* __restrict__ nmsLandmarksOutput, T threadScore, int threadClass, BoxCorner<T> threadBox,
    Landmark<T> threadLandmark, int imageIdx, unsigned int resultsCounter)
{
...
template <typename T, typename Tb>
__global__ void EfficientNMSLandmark(EfficientNMSLandmarkParameters param, const int* topNumData, int* outputIndexData,
    ...
    const T* __restrict__ landmarksInput, const Tb* __restrict__ anchorsInput, int* __restrict__ numDetectionsOutput,
    T* __restrict__ nmsScoresOutput, int* __restrict__ nmsClassesOutput, BoxCorner<T>* __restrict__ nmsBoxesOutput,
    T* __restrict__ nmsLandmarksOutput)
{
    ...
    Landmark<T> threadLandmark[NMS_TILES];
    int boxIdxMap[NMS_TILES];
    for (int tile = 0; tile < numTiles; tile++)
    {
        threadState[tile] = 0;
        boxIdx[tile] = thread + tile * blockDim.x;
        MapNMSLandmarkData<T, Tb>(param, boxIdx[tile], imageIdx, boxesInput, landmarksInput, anchorsInput, topClassData,
            topAnchorsData, topNumData, sortedScoresData, sortedIndexData, threadScore[tile], threadClass[tile],
            threadBox[tile], threadLandmark[tile], boxIdxMap[tile]);
    }
    ...
    for (int i = 0; i < numSelectedBoxes; i++)
    {
            ...
            else if (threadState[tile] == 0)
            {
                ...
                else
                {
                    ...
                    if (write)
                    {
                        ...
                        resultsCounter++;
                        WriteNMSLandmarkResult<T>(param, numDetectionsOutput, nmsScoresOutput, nmsClassesOutput,
                            nmsBoxesOutput, nmsLandmarksOutput, threadScore[tile], threadClass[tile], threadBox[tile],
                            threadLandmark[tile], imageIdx, resultsCounter);
                    }
                }
            }
            else
        ...
        int testClass;
        BoxCorner<T> testBox;
        Landmark<T> testLandmark;
        int testBoxIdxMap;
        MapNMSLandmarkData<T, Tb>(param, i, imageIdx, boxesInput, landmarksInput, anchorsInput, topClassData,
            topAnchorsData, topNumData, sortedScoresData, sortedIndexData, testScore, testClass, testBox, testLandmark,
            testBoxIdxMap);
...
template <typename T>
cudaError_t EfficientNMSLandmarkLauncher(EfficientNMSLandmarkParameters& param, int* topNumData, int* outputIndexData,
    int* outputClassData, int* sortedIndexData, T* sortedScoresData, int* topClassData, int* topAnchorsData,
    const void* boxesInput, const void* landmarksInput, const void* anchorsInput, int* numDetectionsOutput,
    T* nmsScoresOutput, int* nmsClassesOutput, void* nmsBoxesOutput, void* nmsLandmarksOutput, cudaStream_t stream)
{
    ...
    {
        EfficientNMSLandmark<T, BoxCorner<T>><<<gridSize, blockSize, 0, stream>>>(param, topNumData, outputIndexData,
            outputClassData, sortedIndexData, sortedScoresData, topClassData, topAnchorsData,
            (BoxCorner<T>*) boxesInput, (T*) landmarksInput, (BoxCorner<T>*) anchorsInput, numDetectionsOutput,
            nmsScoresOutput, nmsClassesOutput, (BoxCorner<T>*) nmsBoxesOutput, (T*) nmsLandmarksOutput);
    }
    else if (param.boxCoding == 1)
    {
        // Note that nmsBoxesOutput is always coded as BoxCorner<T>, regardless of the input coding type.
        EfficientNMSLandmark<T, BoxCenterSize<T>><<<gridSize, blockSize, 0, stream>>>(param, topNumData,
            outputIndexData, outputClassData, sortedIndexData, sortedScoresData, topClassData, topAnchorsData,
            (BoxCenterSize<T>*) boxesInput, (T*) landmarksInput, (BoxCenterSize<T>*) anchorsInput, numDetectionsOutput,
            nmsScoresOutput, nmsClassesOutput, (BoxCorner<T>*) nmsBoxesOutput, (T*) nmsLandmarksOutput);
    }
...
__global__ void EfficientNMSLandmarkFilterSegments(EfficientNMSLandmarkParameters param,
...
template <typename T>
__global__ void EfficientNMSLandmarkFilter(EfficientNMSLandmarkParameters param, const T* __restrict__ scoresInput,
...
template <typename T>
__global__ void EfficientNMSLandmarkDenseIndex(EfficientNMSLandmarkParameters param, int* __restrict__ topNumData,
...
template <typename T>
cudaError_t EfficientNMSLandmarkFilterLauncher(EfficientNMSLandmarkParameters& param, const T* scoresInput,
    int* topNumData, int* topIndexData, int* topAnchorsData, int* topOffsetsStartData, int* topOffsetsEndData,
    T* topScoresData, int* topClassData, cudaStream_t stream)
{
    ...
    if (param.scoreThreshold < kernelSelectThreshold)
    {
        ...
        PLUGIN_CHECK_CUDA(cudaMemcpyAsync(topScoresData, scoresInput, param.batchSize * param.numScoreElements * sizeof(T),
            cudaMemcpyDeviceToDevice, stream));

        EfficientNMSLandmarkDenseIndex<T><<<gridSize, blockSize, 0, stream>>>(param, topNumData, topIndexData,
            topAnchorsData, topOffsetsStartData, topOffsetsEndData, topScoresData, topClassData);
    }
    else
    {
        EfficientNMSLandmarkFilter<T><<<gridSize, blockSize, 0, stream>>>(
            param, scoresInput, topNumData, topIndexData, topAnchorsData, topScoresData, topClassData);

        EfficientNMSLandmarkFilterSegments<<<1, param.batchSize, 0, stream>>>(
...
template <typename T>
size_t EfficientNMSLandmarkSortWorkspaceSize(int batchSize, int numScoreElements)
{
    size_t sortedWorkspaceSize = 0;
    cub::DoubleBuffer<T> keysDB(nullptr, nullptr);
    cub::DoubleBuffer<int> valuesDB(nullptr, nullptr);
    cub::DeviceSegmentedRadixSort::SortPairsDescending(nullptr, sortedWorkspaceSize, keysDB, valuesDB, numScoreElements,
        batchSize, (const int*) nullptr, (const int*) nullptr);
    return sortedWorkspaceSize;
}

size_t EfficientNMSLandmarkWorkspaceSize(int batchSize, int numScoreElements, int numClasses, DataType datatype)
{
    size_t total = 0;
    ...
    {
        size = EfficientNMSLandmarkSortWorkspaceSize<__half>(batchSize, numScoreElements);
        total += size + (size % align ? align - (size % align) : 0);
    }
    else if (datatype == DataType::kFLOAT)
    {
        size = EfficientNMSLandmarkSortWorkspaceSize<float>(batchSize, numScoreElements);
        total += size + (size % align ? align - (size % align) : 0);
    }
...
template <typename T>
T* EfficientNMSLandmarkWorkspace(void* workspace, size_t& offset, size_t elements)
{
...
template <typename T>
pluginStatus_t EfficientNMSLandmarkDispatch(EfficientNMSLandmarkParameters param, const void* boxesInput,
    const void* scoresInput, const void* landmarksInput, const void* anchorsInput, void* numDetectionsOutput,
    void* nmsBoxesOutput, void* nmsScoresOutput, void* nmsClassesOutput, void* nmsLandmarksOutput, void* workspace,
    cudaStream_t stream)
{
    ...
    int* topNumData = EfficientNMSLandmarkWorkspace<int>(workspace, workspaceOffset, countersTotalSize);
    ...
    int* topIndexData
        = EfficientNMSLandmarkWorkspace<int>(workspace, workspaceOffset, param.batchSize * param.numScoreElements);
    int* topClassData
        = EfficientNMSLandmarkWorkspace<int>(workspace, workspaceOffset, param.batchSize * param.numScoreElements);
    int* topAnchorsData
        = EfficientNMSLandmarkWorkspace<int>(workspace, workspaceOffset, param.batchSize * param.numScoreElements);
    int* sortedIndexData
        = EfficientNMSLandmarkWorkspace<int>(workspace, workspaceOffset, param.batchSize * param.numScoreElements);
    T* topScoresData
        = EfficientNMSLandmarkWorkspace<T>(workspace, workspaceOffset, param.batchSize * param.numScoreElements);
    T* sortedScoresData
        = EfficientNMSLandmarkWorkspace<T>(workspace, workspaceOffset, param.batchSize * param.numScoreElements);
    size_t sortedWorkspaceSize = EfficientNMSLandmarkSortWorkspaceSize<T>(param.batchSize, param.numScoreElements);
    char* sortedWorkspaceData = EfficientNMSLandmarkWorkspace<char>(workspace, workspaceOffset, sortedWorkspaceSize);
    ...
    // Kernels
    status = EfficientNMSLandmarkFilterLauncher<T>(param, (T*) scoresInput, topNumData, topIndexData, topAnchorsData,
    ...
    status = EfficientNMSLandmarkLauncher<T>(param, topNumData, outputIndexData, outputClassData, indexDB.Current(),
        scoresDB.Current(), topClassData, topAnchorsData, boxesInput, landmarksInput, anchorsInput,
        (int*) numDetectionsOutput, (T*) nmsScoresOutput, (int*) nmsClassesOutput, nmsBoxesOutput, nmsLandmarksOutput,
        stream);
    CSC(status, STATUS_FAILURE);
...
pluginStatus_t EfficientNMSLandmarkInference(EfficientNMSLandmarkParameters param, const void* boxesInput,
    const void* scoresInput, const void* landmarksInput, const void* anchorsInput, void* numDetectionsOutput,
    void* nmsBoxesOutput, void* nmsScoresOutput, void* nmsClassesOutput, void* nmsLandmarksOutput, void* workspace,
    cudaStream_t stream)
{
    ...
    {
        param.scoreBits = -1;
        return EfficientNMSLandmarkDispatch<float>(param, boxesInput, scoresInput, landmarksInput, anchorsInput,
    ...
    else if (param.datatype == DataType::kHALF)
    {
        ...
        return EfficientNMSLandmarkDispatch<__half>(param, boxesInput, scoresInput, landmarksInput, anchorsInput,
            numDetectionsOutput, nmsBoxesOutput, nmsScoresOutput, nmsClassesOutput, nmsLandmarksOutput, workspace,
            stream);
    }
    else
