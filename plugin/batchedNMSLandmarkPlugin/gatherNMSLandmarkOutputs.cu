...
__launch_bounds__(nthds_per_cta) __global__
    void gatherNMSLandmarkOutputs_kernel(const bool shareLocation,
                                         const int numImages,
...
pluginStatus_t gatherNMSLandmarkOutputs_gpu(cudaStream_t stream,
                                            ...
                                            const float scoreShift)
{
    ...
    const int BS = 32;
    const int GS = 32;
    gatherNMSLandmarkOutputs_kernel<T_BBOX, T_SCORE, BS><<<GS, BS, 0, stream>>>(
        shareLocation, numImages, numPredsPerClass, numClasses, topK, keepTopK, (int *)indices,
        (T_SCORE *)scores, (T_BBOX *)bboxData, (T_BBOX *)landData, (int *)numDetections,
        (T_BBOX *)nmsedBoxes, (T_BBOX *)nmsedScores, (T_BBOX *)nmsedClasses,
...
// gatherNMSLandmarkOutputs LAUNCH CONFIG {{{
...
static std::array<nmsOutLaunchConfig, 2> nmsOutLCOptions = {
    nmsOutLaunchConfig(DataType::kFLOAT,
                       DataType::kFLOAT,
                       gatherNMSLandmarkOutputs_gpu<float, float>),
    nmsOutLaunchConfig(DataType::kHALF,
                       DataType::kHALF,
                       gatherNMSLandmarkOutputs_gpu<__half, __half>)};

pluginStatus_t gatherNMSLandmarkOutputs(cudaStream_t stream,
                                        const bool shareLocation,
                                        const int numImages,
                                        const int numPredsPerClass,
                                        const int numClasses,
                                        ...
                                        const float scoreShift)
{
    ...
    for (unsigned i = 0; i < nmsOutLCOptions.size(); ++i) {
        if (lc == nmsOutLCOptions[i]) {
            DEBUG_PRINTF("gatherNMSLandmarkOutputs kernel %d\n", i);
            return nmsOutLCOptions[i].function(
                stream, shareLocation, numImages, numPredsPerClass, numClasses, topK, keepTopK,
                indices, scores, bboxData, landData, numDetections, nmsedBoxes, nmsedScores,
                nmsedClasses, nmsedLandmarks, clipBoxes, scoreShift);
        }
    }
