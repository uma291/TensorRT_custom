                            ...
                            bool caffeSemantics)
{
    ...
    /*
     * shareLocation
     * Bounding box are shared among all classes, i.e., a bounding box could be classified as any
     * candidate class. Otherwise Bounding box are designed for specific classes, i.e., a bounding
     * box could be classified as one certain class or not (binary classification).
     */
    ...
    size_t bboxPermuteSize =
        detectionForwardBBoxPermuteSize(shareLocation, N, perBatchBoxesSize, DT_BBOX);
     ...
     */
    if (!shareLocation) {
        status = permuteData(stream, locCount, numLocClasses, numPredsPerClass, 4, DT_BBOX, false,
                             bboxDataRaw, bboxPermute);
    ...
    if (DT_SCORE == DataType::kHALF)
        totalScoresSize /= 2; // detectionForwardPreNMSSize is implemented in terms of kFLOAT
     ...
     */
    status = permuteData(stream, numScores, numClasses, numPredsPerClass, 1, DT_SCORE, confSigmoid,
                         confData, scores);
    ASSERT_FAILURE(status == STATUS_SUCCESS);
    ...
    if (DT_SCORE == DataType::kHALF)
        postNMSScoresSize /= 2; // detectionForwardPostNMSSize is implemented in terms of kFLOAT
    size_t postNMSIndicesSize =
        detectionForwardPostNMSSize(N, numClasses, topK); // indices are full int32
    ...
    bool flipXY = true;
    // NMS
    status = allClassNMS(stream, N, numClasses, numPredsPerClass, topK, iouThreshold, shareLocation,
                         isNormalized, DT_SCORE, DT_BBOX, bboxData, scores, indices, postNMSScores,
                         postNMSIndices, flipXY, scoreShift, caffeSemantics);
    ASSERT_FAILURE(status == STATUS_SUCCESS);

    // Sort the bounding boxes after NMS using scores
    status = sortScoresPerImage(stream, N, numClasses * topK, DT_SCORE, postNMSScores,
                                postNMSIndices, scores, indices, sortingWorkspace, scoreBits);

    ASSERT_FAILURE(status == STATUS_SUCCESS);

    // Gather data from the sorted bounding boxes after NMS
    status = gatherNMSLandmarkOutputs(stream, shareLocation, N, numPredsPerClass, numClasses, topK,
