...
template <typename Trois, typename Tfeat>
__global__ void roiAlign_kernel(const int imageSize, const int featureCount, const int roiCount,
    const int transformCoords, const bool absCoords, const bool swapCoords,
    const int samplingRatio, const Trois* rois, const Tfeat* P2, const xy_t P2dims,
    Tfeat* pooled, const int poolDims)
{
    const int batch = blockIdx.x;
    const int feature = blockIdx.y;
...
cudaError_t roiAlign(const cudaStream_t stream, const int batchSize, const int imageSize,
    const int featureCount, const int roiCount, const int transformCoords,
    const bool absCoords, const bool swapCoords, const int samplingRatio,
    const void* rois, const void* layers, const xy_t layerDims, void* pooled, const int poolDims)
{
    const dim3 blocks(batchSize, featureCount, roiCount);
    const int threads(min(256, poolDims * poolDims));
