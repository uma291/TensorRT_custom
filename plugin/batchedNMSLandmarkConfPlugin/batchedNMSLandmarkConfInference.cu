...
pluginStatus_t nmsInferenceLandmarkConfNew(cudaStream_t stream, const int N, const int perBatchBoxesSize, const int perBatchScoresSize,
    ...
    bool isNormalized, bool confSigmoid, bool clipBoxes, int scoreBits, bool caffeSemantics)
{
     ...
     */
    status = permuteData(stream, numScores, numClasses, numPredsPerClass, 1, DT_SCORE, confSigmoid, confData, scores);
    ASSERT_FAILURE(status == STATUS_SUCCESS);
    ...
    if (DT_SCORE == DataType::kHALF && scoreBits > 0 && scoreBits <= 10)
        scoreShift = 1.f;
    status = sortScoresPerClass(stream, N, numClasses, numPredsPerClass, backgroundLabelId, scoreThreshold, DT_SCORE,
        scores, indices, sortingWorkspace, scoreBits, scoreShift);
    ...
    status = sortScoresPerImage(stream, N, numClasses * topK, DT_SCORE, postNMSScores, postNMSIndices, scores, indices,
        sortingWorkspace, scoreBits);
